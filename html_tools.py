def nested_list_to_html_table(table:list, buttons:bool = False):
    ...
    for i, row in enumerate(table[1:]):
        ...
        if buttons:
            ...
            <div class="tooltip" data-tip="Delete"><button name='delete_button' value='""" + ','.join([str(x) for x in row]) +"""' class='btn btn-sm btn-outline-dark btn-square p-1'>
            <svg xmlns='http://www.w3.org/2000/svg' class='h-6 w-6' fill='none' viewBox='0 0 24 24' stroke='currentColor' stroke-width='2'>
            <path stroke-linecap='round' stroke-linejoin='round' d='M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16' />
            </svg></button></div>
            <div class="tooltip" data-tip="Update"><button name='update_button' value='""" + ','.join([str(x) for x in row]) +"""' class='btn btn-sm btn-secondary btn-square p-1'>
            <svg xmlns='http://www.w3.org/2000/svg' class='h-6 w-6' fill='none' viewBox='0 0 24 24' stroke='currentColor' stroke-width='2'>
